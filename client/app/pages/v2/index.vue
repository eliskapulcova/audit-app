    ...
    <div class="space-y-6">
      <div class="bg-gradient-to-br from-slate-800 via-slate-800 to-slate-900 rounded-xl p-8 shadow-xl border border-slate-700">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div class="bg-slate-900 p-4 rounded-lg">
            <div class="text-3xl font-bold text-blue-400">{{ stats.avgHealthScore }}</div>
            <div class="text-sm text-slate-400">Avg Health Score</div>
          </div>
          <div class="bg-slate-900 p-4 rounded-lg">
            <div class="text-3xl font-bold text-green-400">{{ stats.totalRepositories }}</div>
            <div class="text-sm text-slate-400">Total Repositories</div>
          </div>
          <div class="bg-slate-900 p-4 rounded-lg">
            <div class="text-3xl font-bold text-red-400">{{ stats.totalCriticalIssues }}</div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <ProjectTile v-for="project in filteredProjects" :key="project.id" :project="project" />
      </div>

      <div v-if="filteredProjects.length === 0" class="text-center py-12 bg-slate-800 rounded-xl border border-slate-700">
...
const stats = computed(() => {
    return {
        ...
        totalRepositories: projectSummaries.reduce((sum, p) => sum + p.repositoryCount, 0),
        totalCriticalIssues: projectSummaries.reduce((sum, p) => sum + p.criticalIssues, 0),
    };
})
</script>