<template>
  ...
  >
    <Column field="file" header="Location">
      <template #body="slotProps">
        <div class="flex items-center gap-2">
          <div
            v-html="getIcon(slotProps.data.component).svg"
            class="w-5 h-5"
          ></div>
    ...
    <Column field="rule" header="Rule">
      <template #body="slotProps">
        <a :href="slotProps.data.ruleDocLink" target="_blank">{{
          slotProps.data.rule
        }}</a>
      </template>
    </Column>
...
const processedIssues = computed(() => {
  return issues.map((issue) => {
    return {
      ...
      location:
        issue.component?.replace(/[^:/]*$/, "").replace(/[:/]$/, "") ?? "",
      ...
      impacts: Object.fromEntries(
        issue.impacts.split(", ").map((impact) => impact.split("=")),
      ),
