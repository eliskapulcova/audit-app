<template>
  <div class="bg-slate-800 rounded-xl p-6 shadow-lg border border-slate-700">
      ...
      <table class="w-full">
        <thead>
          <tr class="border-b border-slate-700">
            ...
            <th class="text-left py-3 px-4 text-sm font-semibold text-slate-400">
              Rule ID
            </th>
            <th class="text-left py-3 px-4 text-sm font-semibold text-slate-400">
              Description
        ...
        <tbody>
          ...
          >
            <td class="py-3 px-4">
              <span :class="['px-2 py-1 rounded text-xs font-semibold border', getSeverityColor(issue.severity)]">
                {{ issue.severity }}
              </span>
            </td>
            <td class="py-3 px-4">
              <span :class="['px-2 py-1 rounded text-xs font-semibold', getToolColor(issue.tool)]">
                {{ issue.tool }}
              </span>
            </td>
            ...
            <td class="py-3 px-4 text-xs text-slate-400 font-mono max-w-xs truncate">
              {{ issue.filePath }}
            </td>
    ...
    <div v-if="sortedIssues.length === 0" class="text-center py-8 text-slate-500">
...
const sortedIssues = computed(() => {
  const filtered = props.issues.filter((issue) => {
    ...
    if (filterTool.value !== 'all' && issue.tool !== filterTool.value) return false;
    return true;
  });
